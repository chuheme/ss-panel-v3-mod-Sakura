...
class XCat
{
    ...
    public function boot()
    {
        switch ($this->argv[1]) {
            ...
            default:
                return $this->defaultAction($this->argv[1]);
        }
    }

    public function defaultAction($argv)
    {
        echo "Error: Invalid operation $argv";
    }
    ...
    public function createAdmin()
    {
        ...
        // write input back
        fwrite(STDOUT, "Enter a password for $email : ");
        $passwd = trim(fgets(STDIN));
        echo "Email: $email, Password: $passwd .\n";
        fwrite(STDOUT, "Do you want to continue? [Y/n] ");
        $y = trim(fgets(STDIN));
        if (strtolower($y) == "y") {
            echo "Starting to create admin account.\n";
            // create admin user
            // do reg user
            $user = new User();
