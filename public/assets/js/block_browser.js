...
var tbsHTML = '<div style="padding: 15px"><h2>请从菜单中选择「从浏览器中打开」</h2></div>';
...
if (navigator.userAgent.toLowerCase().indexOf('miuibrowser') !== -1) {
    ...
    document.getElementsByTagName('body')[0].innerHTML = browserHTML;
    ...
    if ((navigator.userAgent.toLowerCase().indexOf('micromessenger') !== -1) || (navigator.userAgent.toLowerCase().indexOf('wechat') !== -1)) {
        window.alert('为了您的人身安全，我们禁止您使用微信访问本站，我们推荐您通过 Chrome 或 Firefox 浏览器访问本站。');
        bodyEl.innerHTML = tbsHTML;
    } else if (navigator.userAgent.toLowerCase().indexOf('mqqbrowser') !== -1) {
        window.alert('为了您的人身安全，我们禁止您使用手机QQ访问本站，我们推荐您通过 Chrome 或 Firefox 浏览器访问本站。');
        bodyEl.innerHTML = tbsHTML;
    }
